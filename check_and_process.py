...
def check_notion_entry(video_id):
    ...
    payload = {"filter": {"and": [{"property": "Video ID", "rich_text": {"equals": video_id.strip()}}, {"property": "Type", "select": {"equals": "Reel"}}, {"property": "Channel", "relation": {"contains": PHONKSTAX_PAGE_ID}}]}}
...
def main():
    ...
    if not item: sys.exit(0)

    v_id = item['contentDetails']['videoId']
    title = item['snippet']['title']

    if check_notion_entry(v_id):
        with open(os.environ['GITHUB_OUTPUT'], 'a') as f: f.write("exists=true\n")
    else:
        # Save data for the shell script
        with open('video_data.json', 'w') as f:
            json.dump({"video_id": v_id, "title": title}, f)
